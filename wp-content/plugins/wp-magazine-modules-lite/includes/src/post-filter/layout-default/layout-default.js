/**
 * Post Filter block layout default - editor preview
 */
...
class PostFilterDefault extends Component {
    constructor( props ) {
        super(...arguments)
    ...
    render() {
        const { imageSize, blockTitle, blockTitleLayout, postCount, fallbackImage, contentOption, contentType, wordCount, thumbOption, titleOption, dateOption, authorOption , categoryOption, categoriesCount, tagsOption, tagsCount, commentOption, permalinkTarget, buttonOption, buttonLabel, postFormatIcon, postMetaIcon, postButtonIcon, blockColumn, postMargin, postCategory } = this.props.attributes
        ...
        const content = posts.map( ( post, index ) => {
            ...
            return (
                <article id={`post-${post.id}`} class={ `cvmm-post post-format--${getformat}`} style={ styleAttr }>
                    { thumbOption &&
